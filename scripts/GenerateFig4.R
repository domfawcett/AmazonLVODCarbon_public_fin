...
#
#Description:
#This code generates Fig. 4 as presented in the manuscript
#
#Outputs:
#-statistics of gross and net biomass changes and trends for each country and the entire Amazon 
#-Figure 4, Supplementary Information Figures S21, S22
#
#Date: 25/10/2022
...
#forest area and intact forest area
...
#time series of AGC change (2011 to 2019)


deforestedBiomassmapbiomasTS <- cellStats(raster::mask(deforestedBiomassmapbiomasstack,subsetmask)*area(deforestedBiomassmapbiomasstack)*100,sum,na.rm=T)/10^9
deforestedBiomassmapbiomasPlusSDTS <- cellStats(raster::mask(deforestedBiomassmapbiomasPlusSDstack,subsetmask)*area(deforestedBiomassmapbiomasstack)*100,sum,na.rm=T)/10^9
deforestedBiomassmapbiomasMinusSDTS <- cellStats(raster::mask(deforestedBiomassmapbiomasMinusSDstack,subsetmask)*area(deforestedBiomassmapbiomasstack)*100,sum,na.rm=T)/10^9

degradedBiomassTS <- cellStats(raster::mask(degradedBiomassstack,subsetmask)*area(degradedBiomassstack)*100,sum,na.rm=T)/10^9
degradedBiomassPlusSDTS <- cellStats(raster::mask(degradedBiomassPlusSDstack,subsetmask)*area(degradedBiomassPlusSDstack)*100,sum,na.rm=T)/10^9
degradedBiomassMinusSDTS <- cellStats(raster::mask(degradedBiomassMinusSDstack,subsetmask)*area(degradedBiomassMinusSDstack)*100,sum,na.rm=T)/10^9

edgeBiomassTS <- cellStats(raster::mask(edgeBiomassstack,subsetmask)*area(edgeBiomassstack)*100,sum,na.rm=T)/10^9
edgeBiomassPlusSDTS <- cellStats(raster::mask(edgeBiomassPlusSDstack,subsetmask)*area(edgeBiomassPlusSDstack)*100,sum,na.rm=T)/10^9
edgeBiomassMinusSDTS <- cellStats(raster::mask(edgeBiomassMinusSDstack,subsetmask)*area(edgeBiomassMinusSDstack)*100,sum,na.rm=T)/10^9

SFBiomassTS <- cellStats(raster::mask(SFBiomassstack,subsetmask)*area(SFBiomassstack)*100,sum,na.rm=T)/10^9

intactBiomasschangesmoothmeanTS <- cellStats(raster::mask(intactBiomasschangesmoothmeanstack,subsetmask)*area(intactBiomasschangesmoothmeanstack)*100,sum,na.rm=T)/10^9
intactBiomasschangesmoothmaxTS <- cellStats(raster::mask(intactBiomasschangesmoothmaxstack,subsetmask)*area(intactBiomasschangesmoothmaxstack)*100,sum,na.rm=T)/10^9
intactBiomasschangetrendmeanTS <- cellStats(raster::mask(intactBiomasschangetrendmeanstack,subsetmask)*area(intactBiomasschangetrendmeanstack)*100,sum,na.rm=T)/10^9
...
LVODAGCTSmethodsmean <- cellStats(raster::mask(AGC_LVOD_amazoniamethodsmeancrop,subsetmask)*area(AGC_LVOD_amazoniamethodsmeancrop)*100,sum,na.rm=T)/10^9
LVODAGCTSmethodsmeanPlusSD <- cellStats(raster::mask(AGC_LVOD_amazoniamethodsmeancropPlusSD,subsetmask)*area(AGC_LVOD_amazoniamethodsmeancropPlusSD)*100,sum,na.rm=T)/10^9
...
AGCTSSDdf <- data.frame(years=seq(2011.5,2018.5,1),
...
#net LVOD AGC changes and uncertainties: 2011 to 2019
LVODnetAGCchange <- methodmeans[9]-methodmeans[1]

LVODnetAGCTSmethodsmeanPlusSDchange <- LVODAGCTSmethodsmeanPlusSD[9]-LVODAGCTSmethodsmeanPlusSD[1]
LVODnetAGCTSmethodsmeanchange <- LVODAGCTSmethodsmean[9]-LVODAGCTSmethodsmean[1]

LVODnetAGCchangeCCISD <- abs(LVODnetAGCTSmethodsmeanPlusSDchange-LVODnetAGCTSmethodsmeanchange)

LVODnetAGCchangeTSsmoothmean <- LVODAGCTSsmoothmean[9]-LVODAGCTSsmoothmean[1]
LVODnetAGCchangeTSsmoothmax <- LVODAGCTSsmoothmax[9]-LVODAGCTSsmoothmax[1]
LVODnetAGCchangeTStrendmean <- LVODAGCTStrendmean[9]-LVODAGCTStrendmean[1]

LVODnetAGCchangeSD <- sd(c(LVODnetAGCchangeTSsmoothmean,LVODnetAGCchangeTSsmoothmax,LVODnetAGCchangeTStrendmean))

LVODnetAGCchangetotSD <- sqrt(LVODnetAGCchangeCCISD^2+LVODnetAGCchangeSD^2)


#net LVOD AGC changes and uncertainties: 2012 to 2019
LVODnetAGCchange2012 <- methodmeans[9]-methodmeans[2]

LVODnetAGCTSmethodsmeanPlusSDchange2012 <- LVODAGCTSmethodsmeanPlusSD[9]-LVODAGCTSmethodsmeanPlusSD[2]
LVODnetAGCTSmethodsmeanchange2012 <- LVODAGCTSmethodsmean[9]-LVODAGCTSmethodsmean[2]

LVODnetAGCchangeCCISD2012 <- abs(LVODnetAGCTSmethodsmeanPlusSDchange2012-LVODnetAGCTSmethodsmeanchange2012)

LVODnetAGCchangeTSsmoothmean2012 <- LVODAGCTSsmoothmean[9]-LVODAGCTSsmoothmean[2]
LVODnetAGCchangeTSsmoothmax2012 <- LVODAGCTSsmoothmax[9]-LVODAGCTSsmoothmax[2]
LVODnetAGCchangeTStrendmean2012 <- LVODAGCTStrendmean[9]-LVODAGCTStrendmean[2]

LVODnetAGCchangeSD2012 <- sd(c(LVODnetAGCchangeTSsmoothmean2012,LVODnetAGCchangeTSsmoothmax2012,LVODnetAGCchangeTStrendmean2012))

LVODnetAGCchangetotSD2012 <- sqrt(LVODnetAGCchangeCCISD2012^2+LVODnetAGCchangeSD2012^2)#sqrt(methodSDs[1]^2+methodSDs[9]^2)

#net LVOD AGC changes and uncertainties: 2011 to 2012
LVODnetAGCchange2011to2012 <- methodmeans[2]-methodmeans[1]

LVODnetAGCTSmethodsmeanPlusSDchange2011to2012 <- LVODAGCTSmethodsmeanPlusSD[2]-LVODAGCTSmethodsmeanPlusSD[1]
LVODnetAGCTSmethodsmeanchange2011to2012 <- LVODAGCTSmethodsmean[2]-LVODAGCTSmethodsmean[1]

LVODnetAGCchangeCCISD2011to2012 <- abs(LVODnetAGCTSmethodsmeanPlusSDchange2011to2012-LVODnetAGCTSmethodsmeanchange2011to2012)

LVODnetAGCchangeTSsmoothmean2011to2012 <- LVODAGCTSsmoothmean[2]-LVODAGCTSsmoothmean[1]
LVODnetAGCchangeTSsmoothmax2011to2012 <- LVODAGCTSsmoothmax[2]-LVODAGCTSsmoothmax[1]
LVODnetAGCchangeTStrendmean2011to2012 <- LVODAGCTStrendmean[2]-LVODAGCTStrendmean[1]

LVODnetAGCchangeSD2011to2012 <- sd(c(LVODnetAGCchangeTSsmoothmean2011to2012,LVODnetAGCchangeTSsmoothmax2011to2012,LVODnetAGCchangeTStrendmean2011to2012))

LVODnetAGCchangetotSD2011to2012 <- sqrt(LVODnetAGCchangeCCISD2011to2012^2+LVODnetAGCchangeSD2011to2012^2)#sqrt(methodSDs[1]^2+methodSDs[9]^2)
...
pdf(file = "./Figures/FinFigs/Fig4_V18.pdf",   
    width = 10, 
    height = 9) 
...
write.table(countrystatsdfTgC,'D:/RECCAP/stats/countryAGCchangestatsuncertaintiesJRC_V18.csv',sep=',',row.names=F)
write.table(countrystatstrenddfTgC,'D:/RECCAP/stats/countryAGCchangestatsTrenduncertaintiesJRC_V18corr2.csv',sep=',',row.names=F)

#make Figures S5 and S6

pdf(file = "./Figures/FinFigs/Supplementary/FigS5_V18.pdf",   
    width = 16, 
    height = 15) 
grid.arrange(grobs=changeprocessplotlistnoIF[1:9],nrow=3,ncol=3)
dev.off()

pdf(file = "./Figures/FinFigs/Supplementary/FigS6_V18.pdf",   # The directory you want to save the file in
